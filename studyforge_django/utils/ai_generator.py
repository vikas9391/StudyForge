...
[/INST]"""
    ...
    try:
        ...
        if match:
            ...
            validated = [q for q in quiz[:num_questions]
...
def _fallback_quiz(text: str, n: int) -> list:
    sentences = re.split(r'(?<=[.!?])\s+', text.strip())
    out = []
...
[/INST]"""
    ...
    try:
        ...
        if match:
            ...
            validated = [c for c in cards[:num_cards] if "front" in c and "back" in c]
            if validated:
                return validated
    except Exception as e:
...
def _fallback_flashcards(text: str, n: int) -> list:
    sentences = re.split(r'(?<=[.!?])\s+', text.strip())
    cards = [
